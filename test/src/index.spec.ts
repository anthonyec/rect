...
const inputOutputTests = [
  ...
  {
    description: 'handles divisions which are lower and upper bounds that are the same',
  ...
  inputOutputTests.forEach((test) => {
    it(test.description, () => {
      ...
      try {
        ...
        console.log('Actual:', frames);
        assert.fail('Actual output did not match expected output')
      }
    });
  });
});
